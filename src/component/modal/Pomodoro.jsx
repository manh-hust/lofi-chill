...
function Button({ big, css, text, active, handle }) {
  return (
    <div
      className={`${
        big ? "w-1/3" : "w-24 font-medium mx-2"
      } ${css} text white cursor-pointer`}
      onClick={handle}
    >
      <div
        className={`${
          active ? "bg-primary text-black" : "text-current"
...
function SettingPomo({
  ...
  const { alarmOn, setAlarmOn, autoRun, setAutoRun, setInitTimesAuto } =
          ...
          <ChangeTime
            ...
            handleMinus={() => {
              if (defaultTime.pomoTime > 0) {
                setDefaultTime({
                  ...defaultTime,
                  pomoTime: parseInt(defaultTime.pomoTime) - 1,
                });
                setCurrentTime(defaultTime.pomoTime * 60);
              }
            }}
            handlePlus={() => {
              if (!defaultTime.pomoTime)
                setDefaultTime({ ...defaultTime, pomoTime: 0 });
              setDefaultTime({
                ...defaultTime,
                pomoTime: parseInt(defaultTime.pomoTime) + 1,
              });
          ...
          <ChangeTime
            ...
            handleMinus={() => {
              if (defaultTime.breakTime > 0)
                setDefaultTime({
                  ...defaultTime,
                  breakTime: parseInt(defaultTime.breakTime) - 1,
                });
            }}
            handlePlus={() => {
              setDefaultTime({
          ...
          <ChangeTime
            ...
            handleMinus={() => {
              if (defaultTime.longTime > 0)
                setDefaultTime({
                  ...defaultTime,
                  longTime: parseInt(defaultTime.longTime) - 1,
                });
            }}
            handlePlus={() => {
              setDefaultTime({
          ...
          <div className="flex flex-col justify-center w-full rounded-lg bg-bg-200 w p-2 mb-4">
            ...
            <div className="flex justify-center mb-4">
              <Button
                ...
                active={true}
                css={
                  "transition-all duration-200 ease-linear hover:opacity-50 "
                }
              ...
              >
                <img
                  src={skipIcon}
                  alt="skip"
                  className="w-full h-full text-white invert"
                />
              </div>
            </div>
          </div>
