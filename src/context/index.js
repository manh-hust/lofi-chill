...
function ThemeProvider({ children }) {
  const [background, setBackground] = useState(() => {
    const link = BACKGROUND_LINKS_LIST.find(
      (item) =>
        item.set === "desk" &&
        item.scene === "scene1" &&
        item.day === true &&
  ...
  const [visiableFocusType, setVisiableFocusType] = useState(
  ...
  const [initTimesAuto, setInitTimesAuto] = useState({
    pomoTimes: 0,
    breakTimes: 0,
    longTimes: 0,
  });

  const setCurrentAndActiveTime = useCallback(
    (type, time) => {
  ...
  useEffect(() => {
      ...
      if (currentTime > 0) {
        ...
        if (activeItem.pomodoro === true) {
          if (initTimesAuto.pomoTimes === 3) {
            ...
            setInitTimesAuto({
              ...initTimesAuto,
              pomoTimes: initTimesAuto.pomoTimes + 1,
            });
          }
          if (alarmOn) {
            refAlarm.current.play();
          ...
          setInitTimesAuto({
            ...initTimesAuto,
            breakTimes: initTimesAuto.breakTimes + 1,
          });
          if (alarmOn) {
            refAlarm.current.play();
          }
    ...
    defaultTime.longTime,
    defaultTime.pomoTime,
    initTimesAuto,
    setCurrentAndActiveTime,
  ]);
...
export { ThemeContext, ThemeProvider, randomAudio };
